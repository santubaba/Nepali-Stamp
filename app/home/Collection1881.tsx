      ...
      <div className="grid items-stretch grid-cols-1 gap-12 px-6 py-16 mx-auto max-w-7xl md:px-12 lg:px-20 md:py-24 md:grid-cols-2 md:gap-16 lg:gap-20">
        ...
        <div className="flex flex-col h-full gap-3">
              ...
              <dl className="grid grid-cols-[auto_1fr] gap-x-6 gap-y-2">
                {metadata.map(({ label, value }) => (
                  <>
                    <dt
                      key={`dt-${label}`}
                      className="text-sm font-meta text-brand-muted whitespace-nowrap"
                    >
                      {label}
                    </dt>
                    <dd
                      key={`dd-${label}`}
                      className="text-sm font-semibold font-meta text-brand-text"
                    >
                      {value}
                    </dd>
                  </>
          ...
          <div className="pt-6 mt-auto">
            <Link
              href="/collections/1881"
              className="
        inline-flex items-center gap-2
        bg-brand-primary text-white
        font-meta text-sm font-semibold tracking-wide
        px-6 py-3 rounded-[var(--radius-md)]
        transition-colors duration-200
        hover:bg-brand-secondary focus-visible:outline-none
